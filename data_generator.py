...
class Siamese_Loader:
    ...
    def __init__(self, path, data_subsets=["train", "val"]):
        ...
        self.file_set = [file for file in (glob.glob(path + '/ecg-id-database-mod/Person_{:02d}/rec_*.png'.format(num)) for num in range(1, 91))]
        
        # defining train and val set
        self.val_set = [89, 74, 13, 1, 38, 52]
    ...
    def make_oneshot_task(self, N, s="val"):
        ...
        support_set[0] = np.array(Image.open(pair_x[0]).resize((h, w)))[:,:,:3] / 255

        j = 0
        for i in pair_x[1:]:
          test_image[j] = np.array(Image.open(i).resize((h, w)))[:,:,:3] / 255
          j += 1

        for i in range(1, N):
          pair_y = np.random.choice(self.file_set[false_idx[i-1]], 1, replace=False)[0]
          support_set[i] = np.array(Image.open(pair_y).resize((h, w)))[:,:,:3] / 255
    ...
    def test_oneshot(self, model, N, k, s="val", verbose=0):
        ...
        for i in range(k):
            ...
            # check accuracy
            acc.append((((probs > 0.5)*1)==targets).mean())
